...
func main() {
	...
	// we strip the static pr"ERROR :", efix, it doesn't exist in our filesystem it's just how we server the static files on the html
	...
	if err := http.ListenAndServe(":8080", Restrict(http.DefaultServeMux.ServeHTTP)); err != nil {
		log.Fatal(err)
	}
}
